...
Registered only when `modules.localimage.enabled` is true.
...
- `size`, `quality`, `style`, and `output_format` must be configured enum values.
- `seed` is optional; omitted seeds are generated by the gateway.
- The tool never accepts provider URLs, workflow paths, node ids, local paths,
  or raw image bytes.
