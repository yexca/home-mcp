...
from modules.image.providers import create_image_provider
from modules.image.service import ImageDeadline, ImageGenerationService, PreparedImageGenerate, prepare_image_generate
from tools.result import success
from transport.request_context import CoreServices, RequestContext

logger = logging.getLogger(__name__)

IMAGE_DEADLINE_MESSAGE = "image job exceeded gateway deadline or was abandoned during restart"


def schedule_image_generate_job(
    *,
    settings: Settings,
    request_id: str,
    ...
    logger.info("image job created", extra={"request_id": request_id, "job_id": job_id})
    thread = threading.Thread(
        target=_run_image_generate_job,
        kwargs={
            "settings": settings,
        ...
        name=f"image-generate-{job_id}",
        daemon=True,
    )
    thread.start()


async def image_generate_background(arguments: dict[str, Any], ctx: RequestContext) -> dict[str, Any]:
    if not ctx.job_id or not ctx.audit_id or not ctx.policy_decision:
        raise internal_error()
    prepared = prepare_image_generate(arguments, ctx)
    schedule_image_generate_job(
        settings=ctx.config,
...
def reconcile_stale_image_jobs(services: CoreServices) -> list[str]:
    image_config = services.config.modules.get("image", {})
    if not bool(image_config.get("enabled", False)):
        return []
    timeout_seconds = image_total_timeout_seconds(services.config)
    grace_seconds = float(image_config.get("stale_job_grace_seconds", 30))
    ...
    job_ids = services.jobs.mark_stale_failed(
        ...
        error_message=IMAGE_DEADLINE_MESSAGE,
    )
    ...
    if job_ids:
        logger.info("reconciled stale image jobs", extra={"job_ids": job_ids})
    return job_ids


def image_total_timeout_seconds(settings: Settings) -> float:
    image_config = settings.modules.get("image", {})
    return float(image_config.get("total_timeout_seconds", settings.limits.get("sync_tool_timeout_seconds", 600)))


def _run_image_generate_job(
    *,
    settings: Settings,
    request_id: str,
    caller: CallerIdentity,
    ...
    timeout_seconds = image_total_timeout_seconds(settings)
    deadline = ImageDeadline.after(timeout_seconds)
    ...
    try:
        ...
        logger.info("image job finished", extra={"request_id": request_id, "job_id": job_id})
        ...
        logger.debug("unhandled image job error", extra={"request_id": request_id, "job_id": job_id}, exc_info=True)
...
def _run_with_deadline(
    ...
    def target() -> None:
        try:
            logger.info("image provider request started", extra={"request_id": ctx.request_id, "job_id": ctx.job_id})
            provider = create_image_provider(ctx.config)
            result = ImageGenerationService(provider).generate_prepared(prepared, ctx, deadline)
            logger.info("image artifact persisted", extra={"request_id": ctx.request_id, "job_id": ctx.job_id})
            results.put(("ok", result))
        except BaseException as exc:
            results.put(("error", exc))

    thread = threading.Thread(target=target, name=f"image-provider-{ctx.job_id}", daemon=True)
    thread.start()
    thread.join(timeout=max(0.001, timeout_seconds))
    if thread.is_alive():
        raise GatewayError(PROVIDER_TIMEOUT, IMAGE_DEADLINE_MESSAGE, retryable=True)
    try:
